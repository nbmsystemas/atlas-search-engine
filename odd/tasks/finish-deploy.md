...
- [x] Deploy backend and frontend through authenticated provider tooling or surface the exact external authorization step.
- [x] Update README with verified URLs and run final endpoint/link checks.
- [x] Make English the default documentation language and add Spanish alternatives plus an interactive architecture artifact.
- [x] Diagnose and correct the production API route once the real Render service URL is known.
...
- Backend is live at `https://atlas-search-engine-api.onrender.com`; the custom frontend domain is `https://atlas.nbmsystemas.com/`.
- The initial Render hostname returned `x-render-routing: no-server` because the service did not exist. Using the authorized Render API, service `atlas-search-engine-api` was created in the `atlas` workspace and deployed from `main`.
